...
- # Regression Model
	...
	- ## Cost function code
		...
		- ```python
		  ...
		  ```
- # Gradient Descent
	- We have a cost function $J(w,b)$ which we want to minimize
	- We want the values for $w$ and $b$ for $J$ that give you the smallest value of $J$
	- Gradient descent also works with models that have more than two parameters
	- Start with some guesses for $w,b$, it doesn't matter that much for gradient descent so we can just use $w=0,b=0$
	- We keep changing $w,b$ to reduce $j(w,b)$ until we settle at or near a minimum
	- Linear regression always has a bowl shaped 3d graph
	- Keep taking steps in directions that cause you to go downhill most quickly.
	- ![Screenshot 2022-12-20 at 7.26.08 PM.png](../assets/Screenshot_2022-12-20_at_7.26.08_PM_1671600583685_0.png)
	- Gradient descent has an interesting property where you can wind up in different places depending where you start
	- You can wind up in a local minima, one of the various small valleys.