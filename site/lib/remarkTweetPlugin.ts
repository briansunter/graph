...
// import twitterVerified from './logo.svg?raw';
...
async function fetchAndReplaceTweet(node, index, parent, tweetMatch) {
  try {
    ...
    if (true) {
      const tweetHtml = createTweetHtml(tweetData, tweetMatch[1], tweetMatch[2]);
      const tweetNode = {
        type: 'raw',
        value: tweetHtml
      };
